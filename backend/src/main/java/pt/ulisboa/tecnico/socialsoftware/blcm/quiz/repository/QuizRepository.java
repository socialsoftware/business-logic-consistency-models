...
public interface QuizRepository extends JpaRepository<Quiz, Integer> {
    ...
    @Query(value = "select q.aggregate_id from quizzes q where q.aggregate_id NOT IN (select aggregate_id from quizzes where state = 'DELETED') AND (q.course_execution_aggregate_id = :courseExecutionAggregateId)", nativeQuery = true)
    Set<Integer> findAllAggregateIdsByCourseExecution(Integer courseExecutionAggregateId);

    @Query(value = "select q.aggregate_id from quizzes q, quiz_quiz_questions qqq where q.aggregate_id NOT IN (select aggregate_id from quizzes where state = 'DELETED') AND (q.id = qqq.quiz_id) AND (:questionAggregateId IN (select question_aggregate_id from qqq))", nativeQuery = true)
