...
public interface QuizRepository extends JpaRepository<Quiz, Integer> {
    ...
    @Query(value = "select * from quizzes q where q.aggregate_id = :aggregateId AND q.version <= :maxVersion AND q.state = 'ACTIVE' AND q.version >= (select max(version) from quizzes where aggregate_id = :aggregateId AND version <= :maxVersion)", nativeQuery = true)
